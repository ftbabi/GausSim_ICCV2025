...
def render_gaussian_physdreamer(
    viewpoint_camera,
    pc: GaussianModel,
    pipe,
...
def render_gaussian(
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            ...
            raw_dir_pp = means3D - viewpoint_camera.camera_center.repeat(
                features.shape[0], 1
            )
            if rotation is not None:
                # TODO : check n is equal to dir_pp.shape[0]
                n = rotation.shape[0]
                assert n == raw_dir_pp.shape[0]
                dir_pp = torch.matmul(rotation, raw_dir_pp.unsqueeze(2)).squeeze(2)
            else:
                dir_pp = raw_dir_pp
            dir_pp_normalized = dir_pp / dir_pp.norm(dim=1, keepdim=True)
