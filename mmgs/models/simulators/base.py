...
class BaseSimulator(BaseModule, metaclass=ABCMeta):
    ...
    @abstractmethod
    def aug_test(self, inputs, gt_label, **kwargs):
    ...
    def train_step(self, data_batch, optimizer, num_epoch=0, num_iter=0, **kwargs):
        ...
        Returns:
            dict: It should contain at least 3 keys: ``loss``, ``log_vars``,
                ...
                averaging the logs.
        """
        losses = self(**data_batch, num_epoch=num_epoch, num_iter=num_iter)
        loss, log_vars = self._parse_losses(losses)
        # print(f"{data_batch['meta']['sequence']}, {data_batch['meta']['frame']}")
        
        num_samples = len(data_batch['gt_label']) // len(data_batch['inputs']['cam'])
        outputs = dict(
            loss=loss,
            log_vars=log_vars,
