...
def tensor_container(data, stack=False):
    if isinstance(data, dict):
        rst = {
            key: tensor_container(data, stack=stack)
            for key, val in data.items()
        }
    elif isinstance(data, list):
        rst = [tensor_container(val, stack=stack) for val in data]
    else:
        rst = DataContainer(data, stack=stack)
    return rst
...
def np_tensor(data, float_type=np.float32, int_type=np.int64, to_container=True):
    ...
    #     rst = tuple([np_tensor(val, float_type=float_type, int_type=int_type, to_container=to_container) for val in data])
    else:
...
def normalize(data, stat):
    ...
    for i in range(len(stat)):
        if isinstance(data[i], torch.Tensor):
            ...
            assert stat[i].shape[0] == 1 or bs == stat[i].shape[0], "Need same dimension"
...
def collapse_uv(uv_groups, duplicate=False):
    assert duplicate == False
    vertices = dict()
    faces = []

    for seg_part, val in uv_groups.items():
        v_dict = val['vertices']
        f_list = val['faces']
        faces.extend(f_list)

        for v_idx, v_uv in v_dict.items():
            if v_idx in vertices.keys() and not duplicate:
                continue
            vertices[v_idx] = v_uv
    max_idx = max(vertices.keys())
    verts_list = np.array([vertices[i] for i in range(max_idx+1)])
