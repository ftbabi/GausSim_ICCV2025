...
class Camera(nn.Module):
    '''
        ...
        To device during run time to fit the pipeline
    '''
    def __init__(
        self,
        R: np.ndarray,
        ...
        self.static_img_path = static_img_path
    ...
    def to_device(self, device):
        '''
            Customized func, to fit the pipeline for mmcv
        '''
        self.world_view_transform = self.world_view_transform.to(device)
        self.projection_matrix = self.projection_matrix.to(device)
        self.cam_plane_2_img = self.cam_plane_2_img.to(device)
    ...
    def interpolate(self, next_camera, steps=24):
        '''
            Not used both here and PhysDreamer.
        '''
